...
DRUPAL_MD5=1db5b6bbacbfd735849221a0a9fb982e
  ...
  set -eux; 
  echo "Download Drupal version https://ftp.drupal.org/files/projects/drupal-$DRUPAL_VERSION.tar.gz"
...
# download CiviCRM, specified version defaulting to STABLE
CIVICRM_VERSION=STABLE
CIVICRM_BRANCH=latest

  set -eux;
  cd sites/all/modules/;
  echo "https://download.civicrm.org/$CIVICRM_BRANCH/civicrm-$CIVICRM_VERSION-drupal.tar.gz"
...
ls -la .
#debug
echo "sudo -E -u www-data drush site-install minimal 
...
#debug off
  ...
  if [ -d /var/www/html/sites/all/modules/civicrm/drupal/drush ]; then
    if [ ! -f /var/www/html/sites/default/civicrm_settings.php ]; then
      ...
      # add --ssl=on in the next line if ssl is enabled
      sudo -E -u www-data drush -y civicrm-install --dbhost=vsql --dbname=$CIVICRM_DATABASE --dbpass=$MYSQL_PASSWORD --dbuser=$MYSQL_USER --site_url=$VSITE_DOMAIN --destination=sites/all/modules
      chmod u-w /var/www/html/sites/default
...
#  fi
  # flush out default blocks that CiviCRM sets up
